...
/**
 * WaterDroplets - Liquid Glass Metaball Refraction Component
 * Adapted from CodePen for React Architecture
 * Acts as a fullscreen background overlay layer for the Hero section.
 */
const WaterDroplets = () => {
  ...
  useEffect(() => {
    ...
    let renderer, scene, camera, mat, bgTexture, dropletTex;
    let drops = [];
    let uid = 0;
    ...
    const init = () => {
      ...
      const drawBackground = () => {
        ...
        /* Brutalist Theme Background */
        bgCtx.fillStyle = "#f4efe6"; // var(--bg-primary)
        bgCtx.fillRect(0, 0, w, h);

        /* decorative fluid waves (Brutalist subtle warm variations) */
        bgCtx.save();
        bgCtx.globalAlpha = 0.15;
        for (let i = 0; i < 3; i++) {
          const cx = w * (0.2 + i * 0.3);
          const cy = h * (0.4 + Math.sin(i * 1.5) * 0.2);
          const rg = bgCtx.createRadialGradient(cx, cy, 0, cx, cy, w * 0.4);
          
          if (i === 0) rg.addColorStop(0, "rgba(201, 162, 39, 0.4)"); // yellow hint
          if (i === 1) rg.addColorStop(0, "rgba(26, 25, 23, 0.15)"); // grey ink hint
          if (i === 2) rg.addColorStop(0, "rgba(232, 224, 212, 0.5)"); // surface break
          
          rg.addColorStop(1, "rgba(244, 239, 230, 0)");
          bgCtx.fillStyle = rg;
          bgCtx.fillRect(0, 0, w, h);
        }
        bgCtx.restore();

        /* We leave the explicit text out, or keep abstract tech elements to refract! 
           Let's scatter some brutalist architectural grids or technical markers instead of big text. */
        bgCtx.strokeStyle = "rgba(26, 25, 23, 0.05)";
        bgCtx.lineWidth = 1;
        
        // Draw some grid lines for refraction
        const gridGap = w * 0.05;
        ...
        /* scattered small brutalist tech text */
        const words = [
          "SYS_ON", "100%", "BUFFER", "REFRACT", "TENSION", 
          "GLITCH_NULL", "SURFACE", "v3.0.1", "AWWWARDS"
        ];
        bgCtx.fillStyle = "rgba(26, 25, 23, 0.15)";
        const scatterSize = Math.max(12, Math.round(w * 0.012));
        bgCtx.font = `600 ${scatterSize}px 'Space Mono', monospace`;
        ...
        for (let i = 0; i < words.length; i++) {
          bgCtx.fillText(
            words[i],
            w * (0.1 + (i % 3) * 0.3),
            h * (0.1 + Math.floor(i / 3) * 0.35 + (i % 2) * 0.1),
          );
        }
      ...
      void main(){
        ...
        vec3  tint  = mix(vec3(1.0), vec3(0.96, 0.94, 0.90), depth * 0.45); // Adapted to panna tint

        vec3 glassColor = bgCA * tint * (0.92 + 0.08 * diff)
                        + vec3(1.0) * spec * 0.85
                        + vec3(0.98, 0.97, 0.95) * rim * 0.22
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        zIndex: 0, // Sits deeply behind DOM content
        pointerEvents: 'auto' // Still catches drags beneath transparent DOM!
      }} 
    />
  );
};
